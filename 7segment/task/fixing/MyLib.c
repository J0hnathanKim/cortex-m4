...
void FND_init(){
	...
	GPIO_PORTH_DEN |= 0x0F;   // H0~H3 segment/digit lines
	GPIO_PORTN_DEN |= 0x30;   // N4~N5 digit lines
	GPIO_PORTC_DEN |= 0xF0;   // C4~C7 segment lines
	GPIO_PORTG_DEN |= 0x0F;   // G0~G3 segment lines
}
...
void WRITE_FND(int digit, int seg_out){
	...
	// If seg_out < 0 then turn that digit off (segments and digit select set to 0)
	if(seg_out < 0){
