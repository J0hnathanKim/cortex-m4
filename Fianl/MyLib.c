...
void DrawTriangle_fill(unsigned char *buffer, int x1, int y1, int x2, int y2, int x3, int y3, int color){
    int i, j;
    ...
    // Precompute vectors for barycentric coordinates
    v1x = x2 - x1;
    v1y = y2 - y1;
    v2x = x3 - x1;
    v2y = y3 - y1;

    dot00 = v1x * v1x + v1y * v1y;
    dot01 = v1x * v2x + v1y * v2y;
    dot11 = v2x * v2x + v2y * v2y;

    invDenom = dot00 * dot11 - dot01 * dot01;
    if(invDenom == 0) return; // Degenerate triangle
